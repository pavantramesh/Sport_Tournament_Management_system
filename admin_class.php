...
Class Action {
	...
	function save_venue(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT INTO tournaments SET ".$data);
			if($save){
				$id = $this->db->insert_id;
				$folder = "assets/uploads/tournament_".$id;
				if(!is_dir($folder)){
				...
				if(isset($img)){
					for($i = 0; $i < count($img); $i++){
						...
						$fname = $id."_".strtotime(date('Y-m-d H:i:s'))."_".$imgName[$i];
						file_put_contents($folder."/".$fname, $img[$i]);
					}
				}
			}
		} else {
			// Update existing venue record
			$save = $this->db->query("UPDATE tournaments SET ".$data." WHERE id = ".$id);
			if($save){
				$folder = "assets/uploads/tournament_".$id;
				if(is_dir($folder)){
	...
	function delete_venue(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM tournaments WHERE id = ".$id);
		if($delete){
			$folder = "assets/uploads/tournament_".$id;
			if(is_dir($folder)){
				$files = scandir($folder);
